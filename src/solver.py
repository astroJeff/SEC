def calc_radius(star):
    ...
    for i in range(star.N_cells-1):
        ln_radius[i] = 1/3 * np.log(r[i+1]**3 + 3/4/np.pi * star.dm/star.cells_rho)
    ...
    for k in range(star.N_cells-1):
        ...
        P[k-1] = P[k] + m_bar * (-c.G*star.mass[k]/(4*np.pi*star.r[k]**4))
    ...
    for T in range(star.N_cells-1):
        ...
        T[k-1] = T[k] + m_bar * (gradT * -c.G*star.mass[k]/(4*np.pi*star.r[k]**4) * T_bar/P_bar)
